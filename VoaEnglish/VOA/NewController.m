...
//    NSLog(@"table:%d",tableView.tag);
    return (tableView.tag == 1? (category == 110? [localArray count]: [voasArray count]+2): [classArray count]);}
...
{
    return ((tableView.tag == 1)? ( category == 110? (isiPhone?80.0f:160.0f):(([indexPath row]<[voasArray count])?(isiPhone?80.0f:160.0f):(([indexPath row]==[voasArray count]+1)?1.0f:(isiPhone?28.0f:48.0f)))): (isiPhone?25.0f:40.0f));
}
...
- (void)requestSoundStarted:(ASIHTTPRequest *)request
{
   ...
   VOAView *voa;
    ...
    if (![VOADetail isExist:request.tag]) {
        NSString *url = [NSString stringWithFormat:@"http://apps.iyuba.com/voa/textChangSuApi.jsp?voaid=%d&format=xml",request.tag];
        //    NSLog(@"catch:%d",voaid._voaid);
        //    ASIHTTPRequest * request = [[ASIHTTPRequest alloc] initWithURL:[NSURL URLWithString:url]];
