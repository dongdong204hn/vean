...
- (void)catchResult:(NSString *) searchWord page:(NSInteger)page{
    ...
    NSString *url = [NSString stringWithFormat:@"http://apps.iyuba.com/voa/searchChangSuApi.jsp?key=%@&format=xml&pages=%d&pageNum=10&parentID=%d&fields=all",searchWord,page,_searchFlg];
    ...
    if ([request.username isEqualToString:@"search" ]) {
        ...
        if (items) {
            ...
            for (DDXMLElement *obj in items) {
                ...
                newVoa._publishTime = [[[obj elementForName:@"PublishTime"] stringValue] isEqual:@" null"]  ? nil :[[obj elementForName:@"PublishTime"] stringValue];
